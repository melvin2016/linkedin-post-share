...
export default class LinekdinPostShare {
    ...
    private async uploadImage(image: Blob, uploadUrl: string) {
        try {
            ...
            if (e instanceof AxiosError) {
                console.log("Cannot upload image. Error: ", e.cause);
                return;
            }
            console.log("Something went wrong. Error: ", e);
        }
    ...
    async createPostWithImage(post: string, image: Blob, imageAlt?: string) {
        ...
        if (!personUrn) {
            console.log("Cannot get person URN");
            return;
        }
        ...
        if (!imageUploadRequest) {
            console.log("Cannot create image upload request");
            return;
        }
        ...
        try {
            ...
            if (data.status != 201) {
                console.log("Image not created. Status code: ", data.status);
                return false;
            }
            return true;
        } catch (e) {
            if (e instanceof AxiosError) {
                console.log("Cannot create post. Error: ", e.response?.data);
                return;
            }
            console.log("Something went wrong. Error: ", e);
        }
